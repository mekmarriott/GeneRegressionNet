...
class TFDNNDoubleEmbeddingModel(TFModel):
  def __init__(self, embed1_shape, embed2_shape, alpha=0.001, regularization=0.0, layer_dims=[], loss='normal', combiner=None, opt='sgd', output_sz=1, activation=None, dropout=False):
    # Inherit all self attributes
    TFModel.__init__(self, embed1_shape[1] + embed2_shape[1], alpha=alpha)
    del self.inputs['x']
    ...
    self.inputs['embed1'] = tf.placeholder(tf.float32, embed1_shape)
    self.inputs['embed2'] = tf.placeholder(tf.float32, embed2_shape)
    self.inputs['y'] = tf.placeholder(tf.float32, [None, output_sz])
    if dropout:
      self.inputs['keep_prob'] = tf.placeholder(tf.float32, [1])

    layers = [embed1_shape[1] + embed2_shape[1]] + layer_dims + [output_sz]
    self.W = {}
    self.b = {}
    ...
    self.embed1_input = tf.nn.embedding_lookup_sparse(self.inputs['embed1'], sparse_input, None, combiner=combiner)
    self.embed2_input = tf.nn.embedding_lookup_sparse(self.inputs['embed2'], sparse_input, None, combiner=combiner)
    self.embed_input = tf.concat([self.embed1_input, self.embed2_input], -1)
    self.layer_inputs = {
        0: self.embed_input
    }
    # Fill in layers
