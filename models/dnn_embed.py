...
class TFDNNEmbeddingModel(TFModel):
  def __init__(self, embed_shape, alpha=0.001, regularization=0.0, layer_dims=[], loss='normal', combiner=None, opt='sgd', output_sz=1, activation=None, dropout=False):
  ...
  def get_embedded_inputs(self, feed):
    _feed = self.translate_feed(feed)
    return self.sess.run(self.embed_input, feed_dict=_feed)
